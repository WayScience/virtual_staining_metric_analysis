...
class Writer:
    ...
    Simple parquet shard writer that accumulates data rows upon add_row()
        ...
        specified shard size or when close() is called.
    This shard writer is intended for writing consistent parquet shards
        with the same known schema. The internal overwrite flag controls
        ...
        of order, size (of content) and identical writer shardsize setting.
    """
    ...
    def __init__(
        ...
        :param shardsize: Number of rows to accumulate before writing a shard.
    ...
    def add_row(self, row: dict[str, Any], schema: pa.Schema | None = None) -> None:
        ...
        If the buffer reaches the configured shard size, it will be flushed to disk.
