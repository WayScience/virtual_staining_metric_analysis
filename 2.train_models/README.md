...
The foreground mask is an Otsu threshold computed per reference image over a fixed intensity range, so a given reference always yields the same mask regardless of how images are batched during evaluation.
Foreground SSIM averages the SSIM similarity map over the masked pixels, and Foreground PSNR masks only the mean squared error while keeping the full data range as the peak, so both remain on the same scale as their unmasked counterparts.
Images with no foreground (an unsplittable Otsu histogram) yield NaN rather than a fabricated score.
